...
import { addfinaltotal } from "@/redux/slices/orderSlice";
...
export default function Page() {
  ...
  const getTotalDiscount = () => {
    return carts.reduce((acc, item) => acc + item?.discount?.discount || 0, 0);
  };
  ...
  const getFinalTotal = () => {
    return getsubTotal() - getDiscountAmount() + DeliveryFee;
  };
  ...
  const handleCheckout = () => {
    if (localStorage.getItem("token")) {
      if (carts.length !== 0) {
        ...
        toast.warning("please add products to cart before going to checkout");
      }
    } else {
  ...
  return (
    <section className="px-2 md:px-32 flex flex-col  ">
      <Toaster/>
      <div className="flex items-center gap-3 py-5 border-t">
        <Link href="/" className="text-gray-500 font-normal text-base">
          Home
        </Link>
        <svg
          width="7"
        ...
        <Link href="/carts" className=" font-normal text-base">
          Cart
        </Link>
      </div>
      ...
      <div className="flex flex-col  lg:flex-row justify-between gap-3">
        <div className=" px-5 border rounded-2xl  flex flex-col min-h-32 h-fit  w-full">
          {carts.length == 0 && (
            <>
              <div className="flex flex-col  items-center justify-center  w-full">
                ...
                <Link
                  href={"/category"}
                  className=" font-bold text-base bg-black text-white py-2 rounded-full my-2 px-10"
                >
                  continue Shopping
                </Link>
              </div>
          ...
          {carts.map((item, index) => (
            <div key={item._id}>
              <div className="flex  gap-5  my-5" key={item._id}>
              ...
              {index != carts.length - 1 ? <hr className="my-2" /> : null}
            </div>
          ))}
        </div>

        <div className="flex flex-col p-2 border rounded-2xl w-full">
          ...
          <div className="flex justify-between my-2">
            ...
            <span className="text-2xl font-bold">
              ${carts.length == 0 ? 0 : getFinalTotal()}
            </span>
          </div>

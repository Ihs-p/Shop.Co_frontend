...
const searchParams = useSearchParams();
  ...
  const user:UserInfo['user'] = JSON.parse(localStorage.getItem('user') || 'null')
          ...
          <div className="flex items-center  md:order-2  rtl:space-x-reverse gap-5">
            <div className="flex ">
              <button
                ...
                aria-controls="navbar-user"
                aria-expanded="false"
                className="lg:hidden text-black dark:text-gray-400  focus:outline-0 focus:ring-0   rounded-lg text-sm p-2.5 me-1"
              >
                <svg
                  className="w-5 h-5"
                  aria-hidden="true"
            ...
            <button
              ...
              onClick={()=> token ? setDropdownVisible( prev => !prev )  : redirect('/login')}
            >
